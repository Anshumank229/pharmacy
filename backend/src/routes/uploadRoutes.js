...
// M1 FIX: Import adminOnly from authMiddleware (canonical source, no deprecated isAdmin check)
...
router.post(
  ...
  async (req, res) => {
    try {
      // M2 FIX: Only return safe fields, not the raw file object with server paths
      res.json({
